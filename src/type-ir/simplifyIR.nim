...
import ../utils/checks
proc traverse[T](ir:Readonly[IR],shouldProcess:proc [O](obj:O): auto ,process:proc (obj:T): auto ): IR = 
...
proc simplify(ir:Intersection|Union): Union|Intersection = 
  ...
  var map = Map()
var state = new(idx= 0,map= map,level= - 1,mustBeTrue= Set())  var expression = visit(ir,state)
  ...
  for config in bitConfigs.mitems:
    ...
    if childTypes.len == 1:
      ...
      var intersection:Intersection = newIntersection(type= "intersection",childTypes= childTypes)      irConfigs.add(intersection)
    else:
      discard
  if irConfigs.len == 1:
    ...
    var union:Union = newUnion(type= "union",childTypes= irConfigs)    return union
  else:
    discard
